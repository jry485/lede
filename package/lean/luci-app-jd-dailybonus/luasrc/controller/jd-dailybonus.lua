...
function index() 
    ...
    entry({"admin", "services", "jd-dailybonus", "check_serverchan"}, call("check_serverchan")) -- 执行serverchan测试
end
...
function check_serverchan()
	...
	if key ~= " " then
		...
		if serverurl~= "scu" then
        	uci:set("jd-dailybonus", '@global[0]', 'dingding', key)
		else
		...
		e.error =luci.sys.call("/usr/share/jd-dailybonus/newapp.sh -m")
	else
		e.error = 1
	end
