...
m=Map("pushserver",translate("PushServer"),
translate("「推送服务」，英文名「PushServer」，是一款从服务器推送报警信息和日志到钉钉的工具。<br /><br />如果你在使用中遇到问题，请到这里提交：")
.. [[<a href="https://github.com/tty228/luci-app-serverchan" target="_blank">]]
.. translate("github 项目地址")
.. [[</a>]]
)

m:section(SimpleSection).template  = "pushserver/status"

s=m:section(NamedSection,"pushserver","pushserver",translate(""))
...
--基本设置
a=s:taboption("basic", Flag,"pushserver_enable",translate("启用"))
a.default=0
a.rmempty = true
...
a=s:taboption("basic", Value,"sctkey",translate('SCKEY'), translate("Serverchan Sckey").."<br>调用代码获取<a href='https://sc.ftqq.com/' target='_blank'>点击这里</a><br><br>")
a.rmempty = true
a:depends("send_tg","")
...
--设备状态
a=s:taboption("content", ListValue,"pushserver_ipv4",translate("ipv4 变动通知"))
a.rmempty = true
a.default=""
a:value("",translate("关闭"))
...
a:depends({pushserver_ipv4="2"})
a.description = translate("<br/>会因服务器稳定性、连接频繁等原因导致获取失败<br/>如接口可以正常获取 IP，不推荐使用")

a=s:taboption("content", ListValue,"pushserver_ipv6",translate("ipv6 变动通知"))
a.rmempty = true
a.default="disable"
...
a:depends({pushserver_ipv6="2"})
a.description = translate("<br/>会因服务器稳定性、连接频繁等原因导致获取失败<br/>如接口可以正常获取 IP，不推荐使用")

a=s:taboption("content", Flag,"pushserver_up",translate("设备上线通知"))
a.default=1
a.rmempty = true

a=s:taboption("content", Flag,"pushserver_down",translate("设备下线通知"))
a.default=1
a.rmempty = true
...
a:depends("send_mode","1")
a:depends("send_mode","2")
a.placeholder = "OpenWrt By W_Y_CPP 路由状态："
...
luci.sys.call("cbi.apply")
        luci.sys.call("/usr/bin/pushserver/pushserver send &")
end

--免打扰
a=s:taboption("disturb", ListValue,"pushserver_sheep",translate("免打扰时段设置"),translate("在指定整点时间段内，暂停推送消息<br/>免打扰时间中，定时推送也会被阻止。"))
a.rmempty = true
...
a.default=0
a.datatype=uinteger
a:depends({pushserver_sheep="1"})
a:depends({pushserver_sheep="2"})
...
a = s:taboption("disturb", DynamicList, "pushserver_whitelist", translate("忽略列表"))
nt.mac_hints(function(mac, name) a :value(mac, "%s (%s)" %{ mac, name }) end)
a.rmempty = true
a:depends({macmechanism="allow"})

a = s:taboption("disturb", DynamicList, "pushserver_blacklist", translate("关注列表"))
nt.mac_hints(function(mac, name) a:value(mac, "%s (%s)" %{ mac, name }) end)
a.rmempty = true
a:depends({macmechanism="block"})

a = s:taboption("disturb", ListValue, "pushserver_interface", translate("接口名称"))
