...
m=Map("pushserver",translate("提示："),
translate("如果你不了解这些选项的含义，请不要修改这些选项"))

s = m:section(TypedSection, "pushserver", "高级设置")
s.anonymous = true
s.addremove = false
...
a.write = function()
	luci.sys.call("/usr/bin/pushserver/pushserver soc")
	luci.http.redirect(luci.dispatcher.build_url("admin","services","pushserver","advanced"))
end

if nixio.fs.access("/tmp/pushserver/soc_tmp") then
e=s:option(TextValue,"soc_tmp")
e.rows=2
e.readonly=true
e.cfgvalue = function()
	return luci.sys.exec("cat /tmp/pushserver/soc_tmp && rm -f /tmp/pushserver/soc_tmp")
end
end
...
a.description = translate("选项 1 选项 2 不会修改设置，并最多尝试 2 次。<br/>选项 3 会将设置项备份于 /usr/bin/pushserver/configbak 目录，并在失败后还原。<br/>【！！无法保证兼容性！！】不熟悉系统设置项，不会救砖请勿使用")
